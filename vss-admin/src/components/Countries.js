...
const Countries = ({ data, itemsPerPage, setItemsPerPage, startFrom, searchByData }) => {
  ...
  return(
    <div className="wrapper">
      ...
      {slicedData.length > 0 ? <Fragment>
        <table className={`table is-fullwidth is-striped ${classes.hero}`}>
          <thead>
            <tr>
              {columns.map((col, index) => (
                <th  className={classes.hero}
                  key={index}
          ...
          <tbody>
            {slicedData.map(item => (
              <tr key={item.id}>
                <td className={classes.hero}>{item.name}</td>
                <td className={classes.hero}>{item.capital}</td>
                <td className={classes.hero}>{item.iso2}</td>
                <td className={classes.hero}>{item.currency}</td>
                <td className={classes.hero}>{item.phone_code}</td>
              </tr>
            ))}
          </tbody>
        </table>

...
const OrderList = ({ data, statusOptions, itemsPerPage, setItemsPerPage, startFrom }) => {
  ...
  const { 
    ...
    const badge = (text, value, color) => (
      value !== 0 && 
        <td style={{...commonStyles.orderSummary, color}}>
           <span style={{...commonStyles.badge, backgroundColor: color}}>{value}</span>
           <br/>
           <span style={{fontFamily: "hero", color}}>{text}</span>
        </td>        
    )
      ...
      <div className="row">
       <div className={classes.rightJustifyContainer}>    
        <table>
          <tbody>
          <tr>
            {badge ("Registered", orderStatusSummary.regCnt, '#d0ac54')}
            {badge ("Processed", orderStatusSummary.procCnt, '#61a785')}
            {badge ("Shipped", orderStatusSummary.shpCnt, '#628185')}
            {badge ("Completed", orderStatusSummary.cmptCnt, '#cc6a4f')}
            {badge ("Canceled", orderStatusSummary.cancelCnt, '#800000')}
          </tr>
          </tbody>
        </table>
      </div>        
            ...
            {slicedData.map(item => (
              <tr key={item.orderid} style={{ paddingBottom: '1rem'}}>
                ...
                <td style={{...commonStyles.tableRow, textAlign: "center"}}>
                  <button type="button" className="btn btn-warning editbtn mr-1"  onClick={() => openPopup(item)}>
                    <span className="icon"><img src={images.order_white}width="15px" alt={item.title}/></span>
                  </button>
                </td>
              </tr>
            ))}
